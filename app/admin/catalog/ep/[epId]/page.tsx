...
export default function EPDetail() {
  ...
  const handleDeleteSongConfirm = async () => {
    ...
    try {
      const response = await fetch(`/api/singles/${songToDelete.id}`, {
        method: "DELETE",
      });
      ...
      <div className="px-[10%] py-14">
        <div className="mb-8">
          ...
          <div className="flex gap-8 items-start">
            <img src={ep.coverImage} alt={ep.name} className="w-64 h-64 object-cover rounded-lg" />
            <div className="flex-1">
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <h1 className="text-4xl font-light tracking-tighter mb-4">{ep.name}</h1>
                  {ep.description && <p className="text-gray-400 mb-4">{ep.description}</p>}
                  <p className="text-gray-500">
                    {ep.songs?.length || ep.songIds.length} song
                    {ep.songs?.length !== 1 || ep.songIds.length !== 1 ? "s" : ""}
                  </p>
                </div>
                <DropdownMenu>
          ...
          <DialogContent className="bg-[#0F0F0F] border-gray-800 text-white">
            <DialogHeader>
              ...
              <DialogDescription className="text-gray-400">
                Are you sure you want to delete &quot;{ep?.name}&quot;? This will remove the EP but keep the songs. This action cannot be undone.
              </DialogDescription>
            </DialogHeader>
            <DialogFooter>
              <Button
        ...
        <Dialog open={deleteSongDialogOpen} onOpenChange={setDeleteSongDialogOpen}>
          <DialogContent className="bg-[#0F0F0F] border-gray-800 text-white">
            ...
            <DialogFooter>
              ...
              <Button variant="destructive" onClick={handleDeleteSongConfirm}>
                Delete
              </Button>
            </DialogFooter>
          </DialogContent>
